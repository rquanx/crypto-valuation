...
export default function Home() {
  ...
  const handleMove = (fromSlug: string, toSlug: string) => {
    setTracked((prev) => {
      if (fromSlug === toSlug) return prev
      const next = [...prev]
      const fromIndex = next.findIndex((item) => item.slug === fromSlug)
      const toIndex = next.findIndex((item) => item.slug === toSlug)
