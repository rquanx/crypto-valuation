...
const MAX_REQUESTS_PER_MIN = Math.min(
  200,
  Math.max(10, Number(process.env.DEFILLAMA_MAX_REQUESTS_PER_MIN ?? 90)),
);
...
async function fetchJson<T>(url: string): Promise<T> {
  const res = await fetch(url, {
    headers: { "user-agent": "crypto-valuation-bot/0.1" },
    cache: "no-store",
  });
  if (!res.ok) {
    ...
    throw new Error(
      `DefiLlama request failed ${res.status} ${res.statusText}: ${snippet}`,
    );
  }
...
export async function fetchFeesOverview(): Promise<OverviewResponse> {
  return scheduleRequest(() =>
    fetchJson<OverviewResponse>(`${API_BASE}/overview/fees`),
  );
}

export async function fetchProtocolSummary(
  slug: string,
  metric: MetricApiType,
): Promise<SummaryResponse> {
  const query = metric ? `?dataType=${metric}` : "";
  return scheduleRequest(() =>
    fetchJson<SummaryResponse>(`${API_BASE}/summary/fees/${slug}${query}`),
  );
}
