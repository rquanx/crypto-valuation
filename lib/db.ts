...
const DB_PATH =
  ...
  path.join(process.cwd(), "data", "crypto-valuation.db");
...
export function applySchema(db: SqliteDatabase = getDb()): void {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS protocol_metrics (
      ...
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      UNIQUE(protocol_id, metric_type, date),
      FOREIGN KEY(protocol_id) REFERENCES protocols(id) ON DELETE CASCADE
    );
    CREATE INDEX IF NOT EXISTS idx_protocol_metrics_protocol_metric_date
      ON protocol_metrics(protocol_id, metric_type, date);
    ...
    CREATE TABLE IF NOT EXISTS ingest_cursors (
      ...
      metric_type TEXT NOT NULL,
      last_date TEXT,
      UNIQUE(protocol_id, metric_type),
      FOREIGN KEY(protocol_id) REFERENCES protocols(id) ON DELETE CASCADE
    );
